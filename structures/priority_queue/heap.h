...
namespace structures {
	...
	void Heap<T>::push(const int priority, const T& data) 	{
		...
		int currentIndex = list_->size() - 1;
		int parentIndex = getParentIndex(currentIndex);
		while (currentIndex > 0 && (*list_)[currentIndex]->getPriority() < (*list_)[parentIndex]->getPriority()) {
	...
	template<typename T>
	T Heap<T>::pop() 	{
		PriorityQueueItem<T>* bestItem = (*list_)[0];
		...
		while (sonIndex != -1 && (*list_)[currentIndex]->getPriority() > (*list_)[sonIndex]->getPriority()) {
			// vymen data current a parent
	...
	inline int Heap<T>::getGreaterSonIndex(const int index) 	{
		...
		PriorityQueueItem<T>* leftSon = leftIndex < list_->size() ? (*list_)[leftIndex] : nullptr;
		PriorityQueueItem<T>* rightSon = rightIndex < list_->size() ? (*list_)[rightIndex] : nullptr;
